var Symbl=(function(){return{PROJECT_NAME:'',WWW_HOME_DIR:'/',WWW_SCRIPT_DIR:'/',REAL_PROJECT_NAME:'/',GET_QUERY:'',init:function(home_dir,project_name,script_dir,real_project_name,query){this.WWW_HOME_DIR=home_dir;this.PROJECT_NAME=project_name;this.WWW_SCRIPT_DIR=script_dir;this.REAL_PROJECT_NAME=real_project_name;this.QUERY=query;},isForm:function(mForm)
{if((Symbl.isForm.arguments.length==2)&&(typeof(Symbl.isForm.arguments[1])=='object')){obj=Symbl.isForm.arguments[1];}else{obj=document;}
var result=false;switch(typeof(mForm)){case'number':if(obj.forms[mForm])
result=obj.forms[mForm];break;case'string':if(obj.forms[mForm])
result=obj.forms[mForm];break;case'object':if(typeof(mForm.method)!='undefined')
result=mForm;break;default:break;}
return result;},isElement:function(mElement,mForm)
{var mResult=false;if((typeof(mForm)!='undefined')&&(typeof(mForm)!='boolean')){if(oF=Symbl.isForm(mForm)){switch(typeof(mElement)){case'number':if(oF.elements[mElement])
mResult=oF.elements[mElement];break;case'string':if(oF.elements[mElement])
mResult=oF.elements[mElement];break;case'object':if(mElement.form.name==oF.name)
mResult=mElement;break;default:break;}}}else{if((typeof(mElement)=='object')&&(typeof(mElement.form)!='undefined'))mResult=mElement;}
return mResult}};})();
